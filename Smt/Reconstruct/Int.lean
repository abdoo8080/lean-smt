...
def reconstructRewrite (pf : cvc5.Proof) : ReconstructM (Option Expr) := do
  ...
  | .ARITH_PLUS_FLATTEN =>
    ...
    let xs : Q(List Int) ← reconstructTerms 0 q(Int) pf.getArguments[1]!.getChildren
    let w : Q(Int) ← reconstructTerm pf.getArguments[2]!
    let ys : Q(List Int) ← reconstructTerms 0 q(Int) pf.getArguments[3]!.getChildren
    let zs : Q(List Int) ← reconstructTerms 0 q(Int) pf.getArguments[4]!.getChildren
